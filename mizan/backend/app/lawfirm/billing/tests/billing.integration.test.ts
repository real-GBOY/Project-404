...
describe("invoiceTotals (domain)", () => {
  it("total = fees + disbursements + round(fees*vat); balance = total - paid", () => {
    ...
    expect(t).toEqual({ fees: 220_000, disbursements: 4_500, vat: 30_800, total: 255_300, paid: 100_000, balance: 155_300 });
  });
});
...
suite("lawfirm/billing", () => {
  ...
  it("invoice lifecycle draft → issued → sent, with a generated number", async () => {
    ...
    const issued = await asUser(firm.adminId, firm.orgId, () => billing().invoiceAction(inv.id, "issue", firm.adminId));
    expect(issued.status).toBe("issued");
    expect(issued.issuedAt).not.toBeNull();
    const sent = await asUser(firm.adminId, firm.orgId, () => billing().invoiceAction(inv.id, "send", firm.adminId));
    expect(sent.status).toBe("sent");
  });

  it("a non-draft invoice cannot be edited (409 invoice.locked)", async () => {
    const inv = await draftInvoice();
    await asUser(firm.adminId, firm.orgId, () => billing().invoiceAction(inv.id, "issue", firm.adminId));
    await expect(
  ...
  it("payment currency must match the invoice", async () => {
    ...
    await expect(
      asUser(firm.adminId, firm.orgId, () =>
        billing().recordPayment({ invoiceId: inv.id, amount: 1000, currency: "AED", method: "cash" }, firm.adminId),
      ),
  ...
  it("payment cannot exceed the outstanding balance; full payment flips the invoice to paid", async () => {
    ...
    await expect(
      asUser(firm.adminId, firm.orgId, () =>
        billing().recordPayment({ invoiceId: inv.id, amount: 999_999, currency: "EGP", method: "bank_transfer" }, firm.adminId),
      ),
    ).rejects.toMatchObject({ code: "payment.overpayment" });

    await asUser(firm.adminId, firm.orgId, () =>
      billing().recordPayment({ invoiceId: inv.id, amount: 114_000, currency: "EGP", method: "bank_transfer" }, firm.adminId),
    );
  ...
  it("expenses: record → approve", async () => {
    const e = await asUser(firm.adminId, firm.orgId, () =>
      billing().recordExpense({ description: "Court fees", amount: 3200, currency: "EGP" }, firm.adminId),
    );
    expect(e.status).toBe("pending");
    const approved = await asUser(firm.adminId, firm.orgId, () => billing().approveExpense(e.id, firm.adminId));
    expect(approved.status).toBe("approved");
  });

  it("finance summary d slot is empty and invoices tab carries an overdue count", async () => {
    const summary = await asUser(firm.adminId, firm.orgId, () => billing().financeSummary("invoices"));
