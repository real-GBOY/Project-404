...
export default function ClientProfileScreen() {
  ...
  const money = (m: { currency: string; amount: string }[]) =>
    m.length ? formatMoneyList(m).join(" · ") : "—";

  return (
    <View style={styles.screen}>
      <TopBar onBack={() => router.back()} actions={[{ icon: "more_vert", onPress: () => {} }]}>
        <View style={styles.identity}>
          ...
          <View style={{ flex: 1, minWidth: 0 }}>
            ...
            <Text style={styles.meta}>
              ...
              {t("clientSince", {
                date: formatDate(client.createdAt, { month: "long", year: "numeric" }),
              })}
            </Text>
          </View>
        </View>
        <View style={styles.actions}>
          ...
          <IconButton
            ...
            variant="outline"
            size={44}
            onPress={() => notAvailableYet(client.name)}
          />
        </View>
      </TopBar>
        ...
        {client.primaryContact ? (
          <Card radius="lgXl">
            ...
            {client.primaryContact.role ? (
              <Text style={styles.contactRole}>{client.primaryContact.role}</Text>
            ) : null}
            {client.primaryContact.phone ? (
              <Text style={styles.contactLine}>{client.primaryContact.phone}</Text>
            ) : null}
            {client.primaryContact.email ? (
              <Text style={styles.contactLine}>{client.primaryContact.email}</Text>
            ) : null}
          </Card>
        ) : null}
        ...
        <View style={styles.statRow}>
          ...
          <Card radius="lgXl" style={{ flex: 1 }}>
            ...
            <Text style={[styles.statValue, { color: colors.dangerText }]}>
              {money(client.stats.outstanding)}
            </Text>
          </Card>
        </View>
