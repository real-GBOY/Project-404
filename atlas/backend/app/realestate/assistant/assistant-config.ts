...
/**
 * Atlas Copilot configuration — **Atlas-owned, not AURIC Core** (the Core config
 ...
 *
 * Ported from `mizan/backend/app/lawfirm/assistant/assistant-config.ts` — same
 * env var names (a separate process/`.env`, so there is no collision), same
 * defaults. The provider/model are swappable on purpose (see
 * docs/atlas-assistant.md): the orchestration layer, tools, and RBAC are
 * provider-independent, so pointing at a different OpenAI-compatible backend is
 ...
 */
export const ASSISTANT_CONFIG = Symbol("atlas.assistant.config");
...
const schema = z.object({
  ...
  /**
   * Keeps the assistant on-topic (Atlas / this portfolio's own data).
...
export function readAssistantConfig(env: NodeJS.ProcessEnv = process.env): AssistantConfig {
  ...
  if (!parsed.success) {
    const issues = parsed.error.issues
      ...
      .join("\n");
    throw new Error(`Invalid AI / Atlas Copilot configuration:\n${issues}`);
  }
