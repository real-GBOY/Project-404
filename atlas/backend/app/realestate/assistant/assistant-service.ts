...
/**
 * Atlas Copilot's orchestration core: agent loop, conversation persistence,
 * scope gate, audit, observability. Ported from
 * `mizan/backend/app/lawfirm/assistant/assistant-service.ts` — same shape,
 * Atlas's own directory/tools/prompt. See docs/atlas-assistant.md.
 */
@Injectable()
export class AssistantService {
  ...
  async chat(input: ChatInput): Promise<ChatResult> {
    ...
    // ── Scope gate — keep the assistant on Atlas's domain ──────────────────
    ...
    const messages: AiMessage[] = [
      {
        ...
        content: buildSystemPrompt({
          ...toolCtx,
          now: this.clock.now(),
          organizationName: organization?.name ?? "your portfolio",
    ...
    if (writes.length > 0) {
      await this.uow.transaction(() =>
        this.audit.record({
          ...
          action: "realestate.assistant.write",
          resourceType: "realestate_ai_conversation",
  ...
  private async recordQueryAudit(
    ...
    try {
      await this.uow.transaction(() =>
        this.audit.record({
          actorId: userId,
          action: "realestate.assistant.query",
          resourceType: "realestate_ai_conversation",
  ...
  /** Observability — structured, and deliberately free of message/portfolio content. */
  private observe(o: {
    correlationId: string;
    input: ChatInput;
...
/** Pull Atlas-id-shaped values out of a tool call's raw JSON args, for the audit
