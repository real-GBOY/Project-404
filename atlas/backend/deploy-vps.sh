#!/usr/bin/env bash
#
# Byte-for-byte the same script as ../../scripts/deploy-vps.sh (Mizan's) — it
# was already fully generic (SERVICE/PORT/BACKEND_DIR all overridable), so
# Atlas's copy just carries Atlas's own defaults. Keep the two in sync by hand
# if the logic ever changes; there is deliberately no shared symlink so each
# product's deploy is self-contained on the box.
#
# Runs ON the VPS as root, after a fresh build has been shipped into place.
# Idempotent; safe to run by hand:
#
#   sudo bash /opt/atlas/deploy-vps.sh
#
# The deploy has already placed, in the backend directory (this script's own
# location):
#   dist/                fresh backend build (tsc output, Prisma staged inside
#                         dist/atlas/backend/ — see scripts/build.mjs)
#   package.json, package-lock.json   (Mizan/Core's — same shared dependency
#                         set Atlas already runs on; see docs/atlas-deployment.md)
#   deploy-vps.sh        this script
#   deploy.env           SERVICE=atlas / PORT=3100 (written by the deploy)
# and refreshed the web bundle under the nginx root.
#
# It never touches $BACKEND_DIR/.env — secrets live only on the box.
set -euo pipefail

# This script is shipped into the backend directory, so its own location is
# that directory. `deploy.env` (shipped next to it) may override SERVICE / PORT.
